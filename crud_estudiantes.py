...
def leer():
    ...
    for fila in tabla.get_children():
        tabla.delete(fila)  # limpia la tabla visual
...
tk.Button(frame_btns, text="➕ Agregar", bg="#6cf3b6", fg="white", command=agregar, width=12).grid(row=0, column=0, padx=5)
tk.Button(frame_btns, text="✏️ Editar", bg="#72a4eb", fg="white", command=editar, width=12).grid(row=0, column=1, padx=5)
tk.Button(frame_btns, text="🗑️ Eliminar", bg="#520506", fg="white", command=eliminar, width=12).grid(row=0, column=2, padx=5)
...
# Expandir tabla al redimensionar ventana
ventana.grid_rowconfigure(4, weight=1)
ventana.grid_columnconfigure(1, weight=1)

# ── INICIO ─────────────────────────────────────
leer()              # cargar datos al abrir
